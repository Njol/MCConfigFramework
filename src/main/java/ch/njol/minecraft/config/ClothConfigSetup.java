		...
		registerType(Integer.TYPE, (value, defaultValue, field, translatePath, saveConsumer) -> {
			...
			if (field.getAnnotation(ch.njol.minecraft.config.annotations.Color.class) != null) {
				...
				return ConfigEntryBuilderImpl.create()
					       ...
					       .setDefaultValue(defaultValue)
					       .setTextGetter(v -> Text.translatable((!slider.minText().isEmpty() && v <= slider.min() ? slider.minText()
						                                          : !slider.maxText().isEmpty() && v >= slider.max() ? slider.maxText()
							                                            : v + slider.unit())))
					       .setTooltip(tooltip)
					       .setSaveConsumer(saveConsumer)
					       .build();
		...
		registerType(Float.TYPE, (value, defaultValue, field, translatePath, saveConsumer) -> {
			...
			if (slider != null) {
				...
				return ConfigEntryBuilderImpl.create()
					       ...
					       .setTextGetter(l -> Text.translatable(!slider.minText().isEmpty() && l * step - 0.01 * step <= slider.min() ? slider.minText()
						                                          : !slider.maxText().isEmpty() && l * step + 0.01 * step >= slider.max() ? slider.maxText()
							                                            : numberFormat.format(l * step * slider.unitStep()) + slider.unit()))
					       .setTooltip(tooltip)
